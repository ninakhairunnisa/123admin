...
| **Products** | Instant search (name/SKU), filter views (low stock, out of stock, no image, on sale, best/slow sellers), quick create, full edit (prices, scheduled sale, SKU, stock, categories, tags, weight, dimensions, descriptions, status), duplicate, variations editor with images, one-tap stock stepper with configurable steps (e.g. ±1/±5/±10) right from the list — variations included, bulk publish/draft/delete, bulk price %, bulk stock, CSV export, print list |
| **Orders** | Live list, smart search (order #, phone, email, name), status/today/yesterday filters with counts, configurable one-tap quick-status buttons (list + detail), product images on every line item (variations included), image-based product picker, **manual order entry from the phone**: one unified customer search field (name / family name / phone / email at once), inline new-customer creation, line-item editing (add/remove/qty), notes & history, invoice + shipping-label print, bulk status/delete, CSV export |
| **Customers** | Unified single-field search (incl. billing phone), quick customer creation (name + phone), profile with lifetime stats & order history, edit profile/role, block/unblock (sessions revoked instantly), internal notes, last login, CSV export |
